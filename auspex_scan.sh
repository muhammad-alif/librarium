...
echo "[*] Starting Recon on $TARGET - By the will of the Omnissiah"s
...
# Extract any domain names from the specified file, remove duplicates, and add them to /etc/hosts
DOMAINS=$(grep -Eo '([a-zA-Z0-9._-]+\.[a-zA-Z]{2,})' "$DOMAIN_FILE" | grep -v "nmap" | sort -u)

if [ -n "$DOMAINS" ]; then
    echo "[*] Found domain names: $DOMAINS"
